...
__global__ void update_particles_all_to_all(
    Particle* particles,
    int num_particles,
...
void create_particles(std::vector<Particle>& particles, int number, int group_id) {
    ...
    std::uniform_real_distribution<float> dis_z(200, depth - 200);
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        update_particles_all_to_all<<<blocks, threads_per_block>>>(
            device_particles, total_particles, device_matrix, d_group_ids, device_fitness, total_groups
        );
        ...
        for (const auto& p : all_particles) {
            ...
            glVertex3f((p.x - width / 2.0f) * expansion, (p.y - height / 2.0f) * expansion, (p.z - depth / 2.0f) * expansion);
        }
        glEnd();
